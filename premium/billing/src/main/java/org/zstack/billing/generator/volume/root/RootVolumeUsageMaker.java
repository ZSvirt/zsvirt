...
import org.zstack.billing.spendingcalculator.volume.root.RootVolumeCreateUsageExtensionPoint;
 ...
 */
public class RootVolumeUsageMaker implements ResourceUsageMaker {
    private static final CLogger logger = Utils.getLogger(RootVolumeUsageMaker.class);
    ...
    public List<Usage> make(List<String> resourceUuids) {
        ...
        ResourceCreateUsageExtensionPoint point = new RootVolumeCreateUsageExtensionPoint();
        ...
        for (VolumeVO volumeVO : volumeVOS) {
            ...
            if (accountUuid == null) {
                logger.warn(String.format("volume[%s] does not belong to any account", volumeVO.getUuid()));
                continue;
            }

            RootVolumeUsageVO usageVO = new RootVolumeUsageVO();
            ...
            usageVO.setVmUuid(volumeVO.getVmInstanceUuid());
