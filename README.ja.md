...
| 領域 | 計画内容 |
|---|---|
| **セキュリティ** | 仮想マシンディスクの暗号化を追加<br>仮想マシン移行の暗号化を追加 |
| **DRS と仮想マシン移行** | より柔軟な移行オプションを追加：<br>• ディスク単位での移行に対応<br>• SAN 間シナリオでホットマイグレーションとコールドマイグレーションの両方に対応 |
| **ZMigrate** | **ZMigrate 2.1**：<br>• 複雑な移行のリスクを抑える事前チェックを追加<br>• よりシンプルでスマートな移行ワークフロー |
| **運用** | VM の CPU Ready 時間、VM ディスクの読み書きレイテンシ、ネットワークパケット損失率、ホストの CPU Ready 時間のメトリクスを追加<br>外部 Syslog 転送でより詳細なログレベルに対応<br>IOPS とレイテンシを含む NVMe／FC デバイス監視を追加<br>VM コンソールの表示と解像度を改善する QXL ドライバーを VMTools に内蔵 |
| **インベントリ管理** | ストレージと VM の登録機能を同一サイト登録まで拡張 |
