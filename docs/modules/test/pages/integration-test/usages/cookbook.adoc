...
. 在上一步之后，若有新增的api，请执行 `./runMavenProfile apihelper`，此时会生成一个文件到 `tests/testlib-simple/src/main/java/org/zstack/testlib/ApiHelper.groovy`
，重新编译 testlib-simple 工程生效
