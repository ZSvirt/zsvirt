...
/**
 * Created by lining on 2018/3/5.
 */
@Configurable(preConstruction = true, autowire = Autowire.BY_TYPE)
public class MinimumMemoryUsageSortFlow extends AbstractHostSortorFlow {
    private static final CLogger logger = Utils.getLogger(MinimumMemoryUsageSortFlow.class);

    private boolean ignoreError() {
        String instanceOfferingUuid = spec.getVmInstance().getInstanceOfferingUuid();

        // If not set, the default is soft mode
        if (!HostAllocatorSystemTags.MINIMUM_MEMORY_USAGE_HOST_ALLOCATOR_STRATEGY_MODE.hasTag(instanceOfferingUuid)) {
            return true;
        }

        String mode = HostAllocatorSystemTags.MINIMUM_MEMORY_USAGE_HOST_ALLOCATOR_STRATEGY_MODE.getTokenByResourceUuid(instanceOfferingUuid, HostAllocatorSystemTags.MINIMUM_MEMORY_USAGE_HOST_ALLOCATOR_STRATEGY_MODE_TOKEN);
    ...
    private List<String> sortHostByMinimumMemoryUsage(List<String> huuids) {
        if (huuids.isEmpty()) {
        ...
        try {
            data = this.getHostMemoryUsedInPercentData(huuids);
        } catch (Throwable t) {
            logger.error(String.format("query host%s %s data fail", huuids, HostNamespace.MemoryUsedInPercent.getName()), t);
            if (!ignoreError()) {
                throw t;
            }
        }
    ...
    private List<Datapoint> getHostMemoryUsedInPercentData(List<String> huuids) {
        long endTime = TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis());
        long startTime = endTime - PremiumHostAllocatorGlobalConfig.MINIMUM_MEMORY_USAGE_HOST_ALLOCATOR_COLLECT_HOST_DATA_DURATION.value(Integer.class);
        ...
        logger.info(String.format("query host[%s] %s data", hostUuidListString, VmNamespace.MemoryUsedInPercent.getName()));
    ...
    public void sort() {
        ...
        List<String> sortedHostUuids = sortHostByMinimumMemoryUsage(candidates.stream().map(HostInventory::getUuid).collect(Collectors.toList()));

        candidates.clear();
        sortedHostUuids.forEach(huuid -> candidates.add(hosts.get(huuid)));
        logger.debug(String.format("Sorted by MinimumMemoryUsage the hosts %s", candidates.stream().map(c -> c.getUuid()).collect(Collectors.toList())));
    }
