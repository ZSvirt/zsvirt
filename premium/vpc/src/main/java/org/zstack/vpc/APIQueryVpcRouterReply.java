...
public class APIQueryVpcRouterReply extends APIQueryReply {
    ...
    public static APIQueryVpcRouterReply __example__() {
        APIQueryVpcRouterReply reply = new APIQueryVpcRouterReply();

        String defaultL3Uuid = uuid();
        String rootVolumeUuid = uuid();

        VpcRouterVmInventory vm = new VpcRouterVmInventory();
        vm.setName("Test-VM");
        vm.setUuid(uuid());
        ...
        dns.setVpcRouterUuid(vm.getUuid());
        dns.setDns("8.8.8.8");
        vm.setDns(asList(dns));
