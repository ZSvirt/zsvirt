...
import org.zstack.monitoring.items.host.HostDiskIOItem;
 ...
 */
public class PrometheusHostDiskIOItem extends HostDiskIOItem implements PrometheusItem {
    @Override
    ...
    public AlertTextWriter createAlertTextWriter() {
        return new PrometheusHostDiskIOAlertWriter();
    }

    private String makeMetricName(String type, String dir) {
        String metricName = "collectd:collectd_disk_disk_";
    ...
    public AlertRuleWriter.RuleBuilder createAlertRule(MonitorTriggerInventory trigger, TriggerExpression expression) {
        ...
        exprb.append(String.format("sum(%s{hostUuid=\"%s\"})", makeMetricName(type, dir), trigger.getTargetResourceUuid()));
