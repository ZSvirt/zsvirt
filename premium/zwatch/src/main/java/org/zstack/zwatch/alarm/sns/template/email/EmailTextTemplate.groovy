...
import org.zstack.sns.platform.email.EmailMessageMetadata
...
class EmailTextTemplate extends AbstractTextTemplate implements TextTemplate {
    static final String ALARM_ENGLISH_TEMPLATE = '''Alarm ${ALARM_NAME} changes status to ${ALARM_CURRENT_STATUS}
...
'''
    ...
    static final String ALARM_CHINESE_TEMPLATE = '''报警器 ${ALARM_NAME} 状态改变成 ${ALARM_CURRENT_STATUS}
...
报警触发时间: ${ALARM_TIME}
报警资源 IP: ${ALARM_RESOURCE_IP}
报警集群 UUID: ${ALARM_RESOURCE_CLUSTER_UUID}
报警集群名称: ${ALARM_RESOURCE_CLUSTER_NAME}
'''
    static final String RECOVERY_CHINESE_TEMPLATE = '''报警器 ${ALARM_NAME} ${TITLE_ALARM_RESOURCE_NAME} 状态改变成 ${ALARM_CURRENT_STATUS}
...
报警集群名称: ${ALARM_RESOURCE_CLUSTER_NAME}
'''
    static final String RECOVERY_ENGLISH_TEMPLATE = '''Alarm ${ALARM_NAME} ${TITLE_ALARM_RESOURCE_NAME} changes status to ${ALARM_CURRENT_STATUS}
...
'''
    ...
    static final String EVENT_ENGLISH_TEMPLATE = '''Event ${EVENT_NAME} Happened
...
'''
    ...
    static final String EVENT_CHINESE_TEMPLATE = '''事件 ${EVENT_NAME} 发生了
...
'''
    ...
    SNSTopicMessage createMessage(SNSTextTemplateInventory template, AlarmAction.TakeAlarmActionParam actionParam, String locale) {
        ...
        message.metadata = JSONObjectUtil.rehashObject(new EmailMessageMetadata(
                subject: StringTemplateUtils.createStringFromTemplate(subjectText, bindings)),
                LinkedHashMap.class)
