...
public class CreateVolumeBackupJob extends AbstractSchedulerJob<VolumeBackupLongJobParams, APIEvent> {
    private static final CLogger logger = Utils.getLogger(CreateVolumeBackupJob.class);
    ...
    public boolean lastJobIsRunning() {
        LongJobState longJobState = Q.New(LongJobVO.class)
                .eq(LongJobVO_.name, VolumeBackupTracker.TASK_NAME)
    ...
    public CreateVolumeBackupJob(CreateSchedulerJobDescMsg msg) {
        super(msg);
        mergeFields(msg.getParameters());
    }

    public CreateVolumeBackupJob() {
        super();
    }
    ...
    private VolumeBackupLongJobParams buildParams() {
        VolumeBackupLongJobParams params = new VolumeBackupLongJobParams();
        params.setName(getTargetResourceUuid().substring(0, 8) + "-backup-" + new Timestamp(System.currentTimeMillis()).toString());
        params.setVolumeUuid(getTargetResourceUuid());
    ...
    private SubmitLongJobMsg buildMsg(VolumeBackupLongJobParams params) {
        SubmitLongJobMsg msg = new SubmitLongJobMsg();
        msg.setName(VolumeBackupTracker.TASK_NAME);
        msg.setDescription(String.format("submitted by Volume backup scheduler job[uuid:%s]", getUuid()));
        msg.setJobData(JSONObjectUtil.toJsonString(params));
        msg.setAccountUuid(params.getAccountUuid());
        msg.setJobName(APICreateVolumeBackupMsg.class.getSimpleName());
        msg.setTargetResourceUuid(params.getVolumeUuid());
        return msg;
    }
    ...
    private void deleteVolumeBackups(List<String> backupUuids, List<String> backupStorageUuids) {
        List<VolumeBackupDeletionMsg> dmsgs = backupUuids.stream()
                .map(backupUuid -> {
                    ...
                    bus.makeTargetServiceIdByResourceUuid(msg, VolumeBackupConstant.SERVICE_ID, getTargetResourceUuid());
                    return msg;
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                List<String> backupUuids = q(VolumeBackupVO.class)
                        ...
                        .isNull(VolumeBackupVO_.groupUuid)
                ...
                if (!backupUuids.isEmpty()) {
                    deleteVolumeBackups(backupUuids, backupStorageUuids);
                }
            }
        }.execute();
    }
    ...
    private void cleanBackupByCount(long count, List<String> backupStorageUuids) {
        ...
        new SQLBatch() {
            ...
            protected void scripts() {
                ...
                if (offset > 0) {
                    deleteVolumeBackups(backupUuids.subList(0, (int)offset), backupStorageUuids);
                }
            }
        }.execute();
    }
    ...
    private ErrorCode checkVolumeVmState() {
        ...
        Tuple t = Q.New(VolumeVO.class).select(VolumeVO_.vmInstanceUuid, VolumeVO_.isShareable, VolumeVO_.type).eq(VolumeVO_.uuid, getTargetResourceUuid()).findTuple();
        ...
        } else if (!Q.New(VmInstanceVO.class).in(VmInstanceVO_.state, Arrays.asList(VmInstanceState.Running, VmInstanceState.Paused)).eq(VmInstanceVO_.uuid, vmUuid).isExists()) {
    ...
    @Override
    public VolumeBackupLongJobParams buildRequest() {
        return buildParams();
    }

    @Override
    public void execute(VolumeBackupLongJobParams request, ReturnValueCompletion<APIEvent> completion) {
        ...
        VolumeBackupTracker tracker = new VolumeBackupTracker(getTargetResourceUuid());
        ...
        longJobMgr.submitLongJob(msg, new CloudBusCallBack(null) {
            ...
            if (event.isSuccess()) {
                APICreateVolumeBackupEvent evt = (APICreateVolumeBackupEvent) event;
    ...
    protected void fireEvent(APIEvent event) {
        String info;
        String volumeUuid = getTargetResourceUuid();
        SchedulerCanonicalEvents.SchedulerExecutedData data = new SchedulerCanonicalEvents.SchedulerExecutedData();
        if (event.isSuccess()) {
            APICreateVolumeBackupEvent evt = (APICreateVolumeBackupEvent) event;
            String wetherFullBackupInfo = evt.getInventory().getMode() == BackupMode.full ? " full" : "";
            String bsUuid = evt.getInventory().getBackupStorageRefs().get(0).getBackupStorageUuid();
            info = String.format("volume[uuid:%s]%s backup succeeded, bsUuid = %s",
                    volumeUuid, wetherFullBackupInfo, bsUuid);
            logger.info(info);
        } else {
            String wetherFullBackupInfo = getBackupMode() == BackupMode.full ? " full" : "";
            info = String.format("volume[uuid:%s]%s backup failed, error: %s",
                    volumeUuid, wetherFullBackupInfo, event.getError().getDetails());
