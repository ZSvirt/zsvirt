 ...
 */
public class PrometheusVmNetworkIOItem extends VmNetworkIOItem implements PrometheusItem {
    @Override
    ...
    public AlertTextWriter createAlertTextWriter() {
        return new PrometheusVmNetworkIOAlertWriter();
    }

    private String makeMetricName(String dir) {
        String metricName = "collectd:collectd_virt_if_octets";
    ...
    public AlertRuleWriter.RuleBuilder createAlertRule(MonitorTriggerInventory trigger, TriggerExpression expression) {
        ...
        exprb.append(String.format("sum(%s{virt=\"%s\"})", makeMetricName(dir), trigger.getTargetResourceUuid()));
