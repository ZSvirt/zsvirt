...
public class StopVmInstanceJob extends AbstractSchedulerJob {
    ...
    public StopVmInstanceJob(CreateSchedulerJobDescMsg msg) {
        super(msg);
    }
    ...
    @Override
    public StopVmInstanceMsg buildRequest() {
    ...
    public void execute(Object msg, ReturnValueCompletion completion) {
        logger.debug(String.format("run scheduler for job: StopVmInstanceJob; vm uuid is %s", getTargetResourceUuid()));
        bus.send((StopVmInstanceMsg) msg, new CloudBusCallBack(completion) {
            @Override
            public void run(MessageReply reply) {
                ...
                if (reply.isSuccess()) {
                    data.setResultMessage(String.format("Stop vm instance job for vm[uuid:%s] succeed", getTargetResourceUuid()));
                } else {
                    data.setError(reply.getError());
                    data.setResultMessage(String.format("Stop vm instance job for vm[uuid:%s] failed", getTargetResourceUuid()));
                }
                evtf.fire(SchedulerCanonicalEvents.VM_STOP_SCHEDULER_PATH, data);
