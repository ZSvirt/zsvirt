package org.zstack.sns.platform.dingtalk

import org.zstack.header.errorcode.ErrorCode
import org.zstack.sns.platform.dingtalk.SNSDingTalkEndpointInventory

doc {

	title "QuerySNSDingTalkEndpoint"

	field {
		name "success"
		desc ""
		type "boolean"
		since "0.6"
	}
	ref {
		name "error"
		path "org.zstack.sns.platform.dingtalk.APIQuerySNSDingTalkEndpointReply.error"
		desc "错误码，若不为null，则表示操作失败, 操作成功时该字段为null",false
		type "ErrorCode"
		since "2.3"
		clz ErrorCode.class
	}
	ref {
		name "inventories"
		path "org.zstack.sns.platform.dingtalk.APIQuerySNSDingTalkEndpointReply.inventories"
		desc "null"
		type "List"
		since "2.3"
		clz SNSDingTalkEndpointInventory.class
	}
}
