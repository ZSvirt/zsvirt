...
class SdkApiTemplate implements SdkTemplate {
    ...
    def generateFields() {
        ...
        APIMessage msg = (APIMessage)apiMessageClass.getDeclaredConstructor().newInstance()

        def output = []

        OverriddenApiParams oap = apiMessageClass.getAnnotation(OverriddenApiParams.class)
        Map<String, APIParam> overridden = [:]
        if (oap != null) {
            for (OverriddenApiParam op : oap.value()) {
                overridden.put(op.field(), op.param())
            }
        }
        ...
        for (Field f : fields) {
            ...
            APIParam apiParam = overridden.containsKey(f.name) ? overridden[f.name] : f.getAnnotation(APIParam.class)
